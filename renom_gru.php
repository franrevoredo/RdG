...
if (filter_input(INPUT_POST, 'nombre')) {
	...
	$q = "UPDATE `rdg`.`grupos` SET `nombre`= ? WHERE `id_grupo`= ? AND `owner`=?;";
	...
	if ($status) {
	    echo "<script>window.location.assign('gtrabajo.php');</script>";
	} else {
...
<form action="renom_gru.php" method="post"><input type="text" name="nombre" value="<?php echo $valor ?>"><input type="hidden" name="id" value="<?php echo $id ?>"><input id="gobutton" type="submit" value="Cambiar Nombre"> </form>